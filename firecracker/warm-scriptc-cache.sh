...
for dependency in curl python3 truncate mkfs.ext4 e2fsck timeout "$firecracker"; do
...
timeout 120s bash -c '
  while kill -0 "$2" 2>/dev/null; do
    grep -q "RUNE_CACHE_WARM_DONE" "$1" && exit 0
    grep -q "RUNE_CACHE_WARM_FAILED" "$1" && exit 3
    sleep 0.05
  done
  exit 4
' _ "$console_log" "$pid"
...
pid=""
set +e
e2fsck -p "$cache" >/dev/null
fsck_status=$?
set -e
(( fsck_status <= 1 )) || { echo "ScriptC cache filesystem check failed" >&2; exit "$fsck_status"; }
chmod 0444 "$cache"
