...
if [[ "${1:-}" == "--print-profile" ]]; then
  [[ $# -eq 1 ]] || exit 2
  read -r vcpu mem_mib _ <<<"$(rune_invocation_profile)"
  echo "$vcpu $mem_mib"
  exit 0
fi

[[ $# -eq 0 ]] || { echo "usage: $0" >&2; exit 2; }
read -r vcpu default_mem_mib _tmp_mib _cpu_seconds _pid_limit _fd_limit <<<"$(rune_invocation_profile)"
...
api_put() {
  curl --fail --silent --show-error --unix-socket "$api_sock" \
    -X PUT -H 'Content-Type: application/json' -d "$2" "http://localhost$1" >/dev/null
}
api_patch() {
  curl --fail --silent --show-error --unix-socket "$api_sock" \
    -X PATCH -H 'Content-Type: application/json' -d "$2" "http://localhost$1" >/dev/null
}
...
for _ in $(seq 1 1200); do
  if grep -q '^RUNE_READY' "$console_log"; then ready=1; break; fi
  kill -0 "$pid" 2>/dev/null || { cat "$console_log" >&2; exit 1; }
  sleep 0.05
done
[[ "$ready" -eq 1 ]] || { echo "guest did not become ready" >&2; cat "$console_log" >&2; exit 1; }
