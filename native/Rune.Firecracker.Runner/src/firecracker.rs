...
impl WarmVm {
    pub async fn restore(config: Arc<Config>) -> Result<Self> {
        ...
        let mut child = Command::new(&config.firecracker_binary)
            ...
            .kill_on_drop(true)
            .spawn()?;
        if let Err(e) = wait_for_path(&api_path, config.restore_timeout).await {
    ...
    pub async fn invoke(
        ...
        .await
        .context("Rune invocation timed out")?
    }
    async fn invoke_inner(
        ...
        if !handshake.starts_with("OK ") {
            bail!("vsock handshake failed: {}", handshake.trim());
        }
        ...
        if response.len() > MAX_GUEST_RESPONSE {
            bail!("guest response exceeded limit");
        }
        if response.last() == Some(&b'\n') {
            response.pop();
        }
        if response.is_empty() {
            bail!("guest returned no response");
        }
...
async fn wait_for_path(path: &PathBuf, timeout: Duration) -> Result<()> {
    ...
    .await
    .context("timed out waiting for Firecracker path")??;
    Ok(())
}
async fn api_put(
    socket: &PathBuf,
    ...
    tokio::time::timeout(timeout, async {
        ...
        if !status.starts_with("HTTP/1.1 200 ") && !status.starts_with("HTTP/1.1 204 ") {
            bail!("Firecracker API {route} failed: {}", status.trim());
        }
        Ok::<(), anyhow::Error>(())
    })
    .await
    .context("Firecracker API timed out")??;
    Ok(())
}
