...
impl VmPool {
    ...
    pub fn set_target(&self, target: usize) {
        self.target.store(
            target.clamp(self.config.min_vms, self.config.max_vms),
            Ordering::Release,
        );
        self.changed.notify_one();
    }
...
mod tests {
    ...
    fn config() -> Arc<Config> {
        Arc::new(Config {
            ...
            state_root: PathBuf::from("/tmp/rune"),
            min_vms: 1,
            max_vms: 4,
            backlog_per_vm: 2,
    ...
    #[test]
    fn backlog_scales_pool_with_ceiling() {
        let p = VmPool::new(config());
        assert_eq!(p.target_for_backlog(0), 1);
        assert_eq!(p.target_for_backlog(3), 2);
        assert_eq!(p.target_for_backlog(99), 4);
    }
}
