...
impl Config {
    pub fn from_env() -> Result<Self> {
        ...
        let config = Self {
            ...
            state_root: env::var_os("RUNE_FIRECRACKER_ROOT")
                .map(PathBuf::from)
                .unwrap_or_else(|| "/var/lib/rune/firecracker".into()),
        ...
        if config.min_vms == 0 || config.max_vms < config.min_vms || config.backlog_per_vm == 0 {
            bail!("invalid VM pool bounds");
        }
        Ok(config)
    }
    ...
    pub fn snapshot_path(&self) -> PathBuf {
        self.state_root.join("snapshot/vmstate")
    }
    pub fn memory_path(&self) -> PathBuf {
        self.state_root.join("snapshot/memory")
    }
